...
impl Car {
    ...
    pub(super) fn update(&mut self, dt: f32, controller: Option<Controller>) {
        if controller.is_some() {
            ...
            //        -1.0 - Emergency brake
            let accel = ct.get_y_axis() * booster;
            let max_backward_vel = -2.;
            let max_forward_vel = 10.;
            if max_backward_vel < self.velocity && self.velocity < max_forward_vel {
                self.velocity += dt * accel;
            } else {
                if self.velocity < max_backward_vel {
                    self.velocity = max_backward_vel;
                } else if max_forward_vel < self.velocity {
                    self.velocity = max_forward_vel;
                }
            }
            ...
            let steer = ct.get_x_axis() * 0.05 * self.velocity;
            ...
            self.position += Vector3::from_homogeneous(forward).unwrap() * self.velocity * dt;
        }
    }
